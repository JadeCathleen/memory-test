...
CSV.foreach(Rails.root.join(data_filepath), csv_options) do |row|
  ...
  if order_product.nil?
    order_product = OrderProduct.new(order: order, product: product, product_quantity: row['quantity'])
    if order_product.save!
      puts "Add product ##{order_product.product.product_code} to order ##{order_product.order.order_code}"
    end
  end
